...
const E_Navbar = () => {
  ...
  return (
    <div className='bg-white rounded-[0.8rem] mt-7 relative'>
      <div className='flex justify-between items-center p-5 min-w-[50px] max-w-[100%] mx-auto '>
        ...
        <div className='flex justify-center items-center gap-2 '>
          ...
          <button className='bg-black text-white w-[35px] h-[35px] rounded-full p-[5px] font-bold text-[18px]  cursor-pointer' onClick={() => { setdashboard(!dashboard) }}>{details? details.charAt(0).toUpperCase() : ''}</button>
        </div>
        {dashboard === true ? <E_Dashboard /> : <></>}
      </div>
    </div>
  )
}
